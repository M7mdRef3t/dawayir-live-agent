/**
 * FEATURE 2 - COGNITIVE FINGERPRINT + EXPORT AS ART
 */
...
function CognitiveFingerprint({ reportContent = '', sessionId = '', lang = 'ar', size = 200, liveState = 'static', voiceTone = 'calm' }) {
  ...
  useEffect(() => {
    if (!reportContent) return undefined;

    const newText = reportContent.slice(lastLenRef.current).toLowerCase();
    lastLenRef.current = reportContent.length;

    const ahaTriggers = ['فهمت', 'صح', 'بالضبط', 'تصدق', 'أخيرًا', 'آها', 'aha', 'eureka', 'exactly', 'makes sense', 'i see', 'now i get it'];
    const triggered = ahaTriggers.some((word) => newText.includes(word));

    if (triggered && !isShattering) {
      setIsShattering(true);
      const timer = setTimeout(() => setIsShattering(false), 2200);
  ...
  const awScore = Math.max(1, awarenessWords.filter((word) => lowerContent.includes(word)).length);
  const knScore = Math.max(1, knowledgeWords.filter((word) => lowerContent.includes(word)).length);
  const trScore = Math.max(1, truthWords.filter((word) => lowerContent.includes(word)).length);
  ...
  const awarenessPath = useMemo(() => {
    ...
    for (let i = 0; i < awPetals; i += 1) {
      const baseAngle = (i / awPetals) * Math.PI * 2;
      const spread = ((rand() * 0.4) + 0.2) * Math.PI / awPetals;
      const len = maxR * (0.3 + awRatio * 0.5 + rand() * 0.2);
    ...
    return d;
  }, [awPetals, awRatio, cx, cy, maxR, rand, voiceTone]);

  const knowledgePath = useMemo(() => {
    let d = '';
    for (let i = 0; i < knSpokes; i += 1) {
      const angle = (i / knSpokes) * Math.PI * 2 + rand() * 0.3;
      const r1 = maxR * (0.15 + knRatio * 0.3);
      const r2 = maxR * (0.25 + knRatio * 0.5 + rand() * 0.15);
    ...
    return d;
  }, [cx, cy, knRatio, knSpokes, maxR, rand]);
  ...
  const downloadAsArt = () => {
    ...
    img.onload = () => {
      ...
      gradient.addColorStop(0, 'rgba(255,0,229,0.08)');
  ...
  const getDynamicStyles = () => {
    ...
    if (isShattering) {
      ...
      svgAnim = 'animation: cfp-breathe 8s ease-in-out infinite;';
  ...
  return (
    <div className={`cognitive-fingerprint-wrapper cfp-wrapper-${styleId}`} title={lang === 'ar' ? 'بصمتك الإدراكية الخاصة' : 'Your unique cognitive fingerprint'}>
      ...
      <svg
        ...
        role="img"
        aria-label={lang === 'ar' ? 'بصمتك الإدراكية الخاصة' : 'Your unique cognitive fingerprint'}
      ...
      <button className="fingerprint-export-btn" onClick={downloadAsArt} title={lang === 'ar' ? 'احفظ بصمتك كصورة' : 'Save as high-res art'}>
        ⬇ {lang === 'ar' ? 'احفظها كصورة' : 'Save as Art'}
      </button>
    </div>
  );
}
